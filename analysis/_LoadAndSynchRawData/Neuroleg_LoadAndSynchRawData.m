...
savedir  = '\\bmi-nas-01\Contreras-UH\Neural Control of Powered Artifical Legs\EEG-FMRI-WALK DATA\_RAW_SYNCHRONIZED_EEG_FMRI_DATA';
...
EEGEMPTY = EEG;
clearvars -except drive datadir savedir EEGEMPTY
...
for ii = 1:length(subs)
    ...
    for jj = 1:length(eegfiles)
        ...
        event_time = [EEG.event(2:end).latency]./old_srate;
        for kk = 2:length(EEG.event)
            EEG.event(kk).latency = floor(event_time(kk-1).*EEG.srate);
        end
...
for aa = 3 %1:length(subs)
    ...
    eegfiles = dir(fullfile(savedir,[subs{aa} '-T*-eeg.mat']));
    ...
    for bb = 1:length(eegfiles)
        ...
        load(fullfile(savedir,thisfile));
        ...
        if ~strcmpi(splitname{2},'T00') % baseline trials not included
            ...
            %          Find EMG          %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            emgfile = fullfile(savedir,strjoin({splitname{1:2},'emg.mat'},'-'));
            load(emgfile);
            ...
            %          Find GONIO        %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            goniofile = fullfile(savedir,strjoin({splitname{1:2},'gonio.mat'},'-'));
            load(goniofile);
            ...
            %          Find OPAL         %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            opalfile = fullfile(savedir,strjoin({splitname{1:2},'opal.mat'},'-'));
            load(opalfile);
            % Get opal fields
            ...
            %          Find STIM         %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            stimfile = fullfile(savedir,strjoin({splitname{1:2},'stim.mat'},'-'));
            load(stimfile);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    load(fullfile(savedir,strcat(splitname{1},'-chanlocs.mat')));
    ...
    EEGCONCAT.process = {'Cut@ExpStart/Stop','Concatenate','HPFilt@3Hz','Hinf'};
