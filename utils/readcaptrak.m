...
% Get fiducial channels
findNz  = find(strcmpi({chanlocs.labels},'Nz'));
findRPA = find(strcmpi({chanlocs.labels},'RPA'));
findLPA = find(strcmpi({chanlocs.labels},'LPA'));
...
% Remove fiducials
chanlocs([findNz,findRPA,findLPA]) = [];
end
