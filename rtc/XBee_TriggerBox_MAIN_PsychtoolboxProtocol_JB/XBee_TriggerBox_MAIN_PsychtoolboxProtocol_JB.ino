...
//------------------------------------------------------------------------------
//  // ************ //
//  // Check flag 5 //    // For matlab_serial trigger
//  // ************ //
//
//  // NOTE: A change in flag 5 (from matlab_serial interrupt) ONLY creates marker in EEG data (S1 and S2)
//
//  if (flag5 == true)  // trigger by interrupt
//  {
//    int state5 = digitalRead(matlab_serial); // read and store current value (HIGH or LOW)
//
//    flag5 = false; // reset flag
//
//    if (state5 == HIGH)  // serial is not null
//    {
//      // Set pins to high
...
//      // Turn on LED BNC
//      digitalWriteFast(synch_out1, HIGH);
//      //Make marker different from eeg_in1 pulse
//      //delayMicroseconds(100000);
//    }
//    else if (state5 == LOW)  // serial is null
//    {
//      // Set pins to low
...
// ***********************************************************************************************************
// *                                                                                                         *
// *                        ******* Change status of XBee digital pins *******                               *
// *                                                                                                         *
// ***********************************************************************************************************

void setXBeeState(char value) {

  // Remote XBee: AT, Base XBee: API
  Serial1.write(0x7E); // Sync up the start byte
  Serial1.write((byte)0x0); // Length MSB (always 0)
  Serial1.write(0x10); // Length LSB
  Serial1.write(0x17); // 0x17 is the frame ID for sending an AT command
  Serial1.write((byte)0x0); // Frame ID (no reply needed)
  Serial1.write((byte)0x0); // Send the 64 bit destination address
  Serial1.write((byte)0x0); // (Sending 0x000000000000FFFF (broadcast))
  ...
  Serial1.write(0xFF);
  Serial1.write(0xFF);
  Serial1.write(0xFF); // Destination Network
  Serial1.write(0xFE); // (Set to 0xFFFE if unknown)
  Serial1.write(0x02); // Set to 0x02 to apply these changes
  ...
  // Checksum
  long checksum = 0x17 + 0xFF + 0xFF + 0xFF + 0xFE + 0x02 + 'D' + '3' + value;
  Serial1.write(0xFF - (checksum & 0xFF)); // Checksum
