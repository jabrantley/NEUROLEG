...
% * *Date Created :* 20-Mar-2019 20:05:58
...
%%
function varargout = GUI_grbl(varargin)
...
uipanel_save = uipanellist('title','PLAY',...
    ...
    'gap',[0 gvar.margin.gap]);
% Alignment
uialign(container_filelist,container_currdir,'align','southwest','scale',[1 4],'gap',[0 -gvar.margin.gap]);
uialign(uipanel_serial,container_filelist,'align','southwest','scale',[1 2.5],'gap',[0 -2*gvar.margin.gap]);
...
elseif strcmpi(key,'return')
    pushbutton_serialWrite_Callback(hObject,eventdata,handles)
end
...
if  strcmpi(get(mySerial,'status'),'open')
    while get(mySerial, 'BytesAvailable') > 0
