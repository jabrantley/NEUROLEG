...
% * *Date Created :* 30-Oct-2018 00:18:14
...
%%
function GUI_GsheetLog(varargin)
...
uistring={icontext(handles.iconlist.action.updir,''),...
    {'G:\OneDrive\MY-PROJECT',cd,filedir,'C:\'},...
...
uijlist_setfiles(handles.jlistbox_filenameinput,get(handles.combobox_currdir,'selecteditem'),'type',{'.all'});
handles.keyholder = '';
% Set callback
% Keyboar thread
...
if eventinf.Button==1 && eventinf.ClickCount==2 %double left click
    ...
    elseif strcmpi(ext,'.xlsx')
        xlsLogfile= fullfile(currdir,filename);
        [status, sheets, xlFormat] = xlsfinfo(xlsLogfile);        
        xlsread(xlsLogfile, sheets{1})
    end
end
% ==== END
fprintf('DONE: %s.\n',thisFuncName);
