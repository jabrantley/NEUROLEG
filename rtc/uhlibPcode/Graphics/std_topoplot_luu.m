...
for k = 3:2:nargin
    switch varargin{k-2}
        ...
        case 'showtitle'
            titlemode = varargin{k-1}; % Add by Luu 160804 to remove text on top of topoplot.
    end
end
...
if strcmpi(mode, 'apart')         
    for clus = 1: length(cls) % For each cluster requested
        ...
        if len > 0 % A non-empty cluster 
            ...
            ave_grid = rot90(ave_grid); % Luu rot90
            tmp_ave = ave_grid;
...
if strcmpi(mode, 'together') 
    ...
    for k = 1:len 
       if len ~= 1
        ...
        % Luu modify, rotate 90 degree because
        % using custom elc cap file
        luu_topo = STUDY.cluster(cls(k)).topo;        
        toporeplot(rot90(luu_topo), 'style', 'both', 'plotrad',0.5,'intrad',0.5, 'verbose', 'off','colormap', tmpcmap);
%         toporeplot(STUDY.cluster(cls(k)).topo, 'style', 'both', 'plotrad',0.5,'intrad',0.5, 'verbose', 'off','colormap', tmpcmap);
        if strcmpi(titlemode,'on') % Luu
