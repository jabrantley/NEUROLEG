...
function [RAWEEG,RAWEMG,ANGLEVEC] = neuroleg_realtime_train(params,b)
...
RAWEMG = zeros(params.setup.numEMGchans, size(RAWEEG,2));
...
while ~finish
    try
        % Get EMG data
        emgdata = [];
        for ii = 1:params.setup.numEMGchans
            temp = double(b.getdata).*b.EMG_GAIN;
            if ~isempty(temp)
                emgdata = [emgdata; temp(:)'];
                numsampsEMG = [numsampsEMG, size(emgdata,2)];
            end
        end
        endEMG = startEMG + size(emgdata,2) - 1;
        RAWEMG(:,startEMG:endEMG) = emgdata;
        startEMG = endEMG + 1;
        ...
        while ~isempty(tryheader)
            ...
            switch hdr.type
                ...
                case 4       % 32Bit Data block
                    ...
                    startEEG = endEEG + 1;
                    numsampsEEG = [numsampsEEG, size(eegdata,2)];
