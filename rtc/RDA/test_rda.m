...
function test_rda(params)

% DEFINE PARAMETERS
params = neuroleg_realtime_setup;

% Clear persistent variables
clear uhbmi_CleanMotionArtifacts
        ...
        while ~isempty(tryheader)
            ...
            switch hdr.type
                ...
                case 4       % 32Bit Data block
                    ...
                    % Get eogdata data
                    eogdata = eegdata([17,22,28,32],:);
                    % remove EOG from EEG data
                    eegdata([17,22,28,32],:) = [];
                    eegdata = transpose(eegdata);
                    eogdata = transpose(eogdata);
                    % Pass to real time function
                    ...
                    %                         cleandata(ii,:) =  uhbmi_CleanMotionArtifacts(eegdata(ii,:),eye_artifacts(ii,:),params.gamma,params.q,0,params.A,params.B,params.C,params.D);
                    %                     end
                    
                    [cleandata, filtdata, params] = neuroleg_realtime_processing(eegdata,eogdata,params);
                    
                    RAWEEG = [RAWEEG; eegdata];
                    CLEANEEG = [CLEANEEG; cleandata];
                    FILTEEG = [FILTEEG; filtdata];
...
% Filter Raw EEG to remove drift
RAWEEG = filterdata('data',RAWEEG,'srate',params.srate,...
