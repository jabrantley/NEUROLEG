...
datalog = class_Biometrics_64bits('numberofvalues',1,...
    'usech', used_chans, ...
    'sensor', type_chans,...
    'gain', ch_gains);
...
while(1)
    ...
    if ~isempty(datalogval)
        check = datalogval
        timeplot = transpose(linspace(toc_startTime - length(datalogval)*handles.datalog.samplerate/1000,toc_startTime,...
            length(datalogval)))
        %         pretoc = toc_startTime - length(datalogval)*handles.datalog.samplerate/1000
        %         tocTime = toc_startTime
        addpoints(handles.dataLine,...
            timeplot,...
            datalogval);
        set(gca,'XLim',datenum([toc_startTime - handles.npts_dataLine/handles.datalog.samplerate,...
            toc_startTime]));
        drawnow;
    end
end
