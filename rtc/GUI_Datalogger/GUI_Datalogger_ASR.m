...
uhlib = 'D:\GUI_Apps\uhlibPcode'; % Use the uhlibPcode in BBB_v2
% addpath('D:\GUI_Apps\GUI_Datalogger\Includes');
addpath('D:\GUI_Apps\GUI_Datalogger\Includes');
...
datalog = class_Biometrics_64bits_ASR('numberofvalues',1,...
    'usech', used_chans, ...
    'sensor', type_chans,...
    'gain', ch_gains);
...
if strfind(lower(get(hObject, 'string')), 'play')
    ...
    [EMG_data,handles] = handles.datalog.getdata(handles);
...
%     start(handles.mytimer);
    ...
    if get(handles.checkbox_useserial,'value')
        msg = ['e', char(toByte(0,handles.emgRange)),...
            char(10)]; % Send 'e' character for EMG.
        fwrite(handles.mySerial,msg);        
    end
