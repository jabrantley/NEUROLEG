classdef class_Biometrics_64bits < hgsetget;
    % Date 160601.
    ...
    methods (Access = public) %Constructor
        %Constructor
        function this = class_Biometrics_64bits(varargin)
        ...
        function datalogval = getdata(this)
            % First initialise the return values from the DLL functions            % 
            datalogval = NaN(this.numch, 1);
            for i = 1 : this.numch
                ch = this.usech(i)-1;
                pstatus = libpointer('int32Ptr', 0);
                calllib('OnLineInterface64', 'OnLineStatus', ch,...
                    this.ONLINE_GETSAMPLES, pstatus);
                if pstatus.Value > 0
                    nb_get_samples = pstatus.Value; % Get all available samples in the buffer
                    ...
                    values.cbElements = 2;	% 2-byte values
                    values.cLocks = 0;
                    values.rgsabound.cElements = nb_get_samples;
                    values.rgsabound.lLbound = nb_get_samples;
                    values.pvData = int16(1:nb_get_samples);                    
                    calllib('OnLineInterface64', 'OnLineGetData', ch,...
                        floor(nb_get_samples * 1000 / this.samplerate), values, pdataNum);
%                     datalogval(i) = mean(values.pvData)*this.vecgain(i);
%                     datalogval(i) = values.pvData(:)*this.vecgain(i);
                      datalogval = double(values.pvData(:))*this.vecgain(i);
%                     this.buffer_data(i) = datalogval(i);
                else
%                     datalogval = this.buffer_data;
%                     break;
                end
            end
        end
