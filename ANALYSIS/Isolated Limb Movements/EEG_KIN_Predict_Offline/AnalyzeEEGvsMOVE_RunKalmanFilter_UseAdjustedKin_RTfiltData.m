...
fulltime = 0:1/EEG.srate:(trial_duration+2*window_buffer/EEG.srate);
        ...
        bb
            ...
            for cc = 1:size(movetimes{aaa},1)
                ...
                for dd = 1:size(movetimes{aaa}{cc,2},1)
                    ...
                    t2 = move_win(1)+(trial_duration*EEG.srate)- 1/EEG.srate + window_buffer;
                    % Shift time according to computed phase lag
                    temp_time = (t1:t2) + abs(mean(lag2{aa}));
                    % Get data
