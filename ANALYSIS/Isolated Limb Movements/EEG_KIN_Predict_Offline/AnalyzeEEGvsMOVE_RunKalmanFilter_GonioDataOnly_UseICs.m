...
for aa = 3 %1:length(subs)
    ...
    % Get eeg files for each subject
    load(fullfile(datadir, [subs{aa}, '-ALLTRIALSNOFILT-eeg.mat'   ]));
    load(fullfile(rawdir, [subs{aa}, '-ALLTRIALS-stim.mat'  ]));
    % Load movement data
    load(fullfile(rawdir, [subs{aa}, '-ALLTRIALS-gonio.mat' ]));
    ...
    total = 1;
    combos = cell(length(BANDS)*size(EEG.icaweights,1),4);
...
%             end
        ...
        filename = [subs{aa} '_KF_RESULTS_GONIO_' movements{aaa} '_WIN' num2str(num2str(1/update_rate)) '_ICAALL_AUG' num2str(useAug) '_UKF' num2str(useUKF) '.mat'];
        save(filename,'R2_sub_all','R2_sub_mean','predicted_sub','combos');
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %                                                           %
% %        SEGMENT MOVE WINDOW AND TRAIN KALMAN FILTER        %
% %                                                           %
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% all_movetimes = cell(length(subs),1);
% % Loop through each subject, concatenate, and process
...
%     
%     
%     % Loop through each movement and get EEG and gonio data
%     EEGDATA = cell(1,size(STIM,1)*4);
%     GONDATA = cell(1,size(STIM,1)*4);
%     iter = 1;
%     for aaa = 1:length(movements)
%         limb = movements{aaa};
%         movetimes{aaa,1} = cell(size(STIM,1),2);
...
%             
%             % Get gonio data, zscore, normalize
%             tempgon = zscore(GONIO(bb).data(1,:));
%             normdat = abs(tempgon-max(tempgon));
%             
%             % Get movement times from STIM
%             rk_idx      = find(strcmpi(STIM(bb).states,limb)); % Get movements of right knee
%             rk_onset    = STIM(bb).initialDelay + STIM(bb).onsets(rk_idx); % seconds
...
%             rk_samples  = floor(rk_time .* EEG.srate); % sample points
%             
%             % Get each move window
%             for cc = 1:size(rk_samples,1)
%                 EEGDATA{iter} = EEG.data(:,rk_samples(cc,1):rk_samples(cc,2));
%                 GONDATA{iter} = EEG.data(:,rk_samples(cc,1):rk_samples(cc,2));
%             end
%             % Store movement times
%             movetimes{aaa}{bb,1} = find(EEG.trialbreaks==bb);
%             movetimes{aaa}{bb,2} = rk_samples;
