...
for aa = 3%1:length(subs)
    ...
    for aaa = 1:length(movements)% 1:length(movements)
        ...
        filename = [subs{aa} '_KF_RESULTS_TARGET_EachChanRTClean_' movements{aaa} '_WIN' num2str(num2str(1/update_rate)) '_Z' num2str(zscore_data) '_CAR' num2str(car_data) '_AUG' num2str(useAug) '_UKF' num2str(useUKF) '.mat'];
